...
def get_prices_from_url(url):
    ...
    json = "{ \n"
    price_table = soup.find_all('div', class_="pricing-table")
    for price in price_table:
        json +="\t { \n"
        h = price.find('div', class_="pricing-table-header")
        header = h.find('h2').text

        json +="\t\t '"+header+"': { \n"

        p = price.find('span', class_="pricing-table-price").text.strip().split(" ")[0]
        json +="\t\t\t 'price': '"+p+"',\n"

        storage = price.find('ul', class_="pricing-table-list").find_all('li')[3].text.split(" ")[0]

        json+="\t\t\t 'storage': '"+ storage+"',\n"

        database = price.find('ul', class_="pricing-table-list").find_all('li')[4].text.split(" ")[0]
        json += "\t\t\t 'databases': '" + database + "'\n"
...
def extract_beer_list_infos(url):
    import json
    data = requests.ge(url).content
    data = json.load(data)
    beer = data['items']
    beer_urls = [
        'https://www.berrwuld.com' + b['contentReference'] for b in beer
    ]

   # from multiprocessing import pool
   # pool = Pool(processes = 8)
   # result = pool.map(extract_beer_infos, beer_urls)

    return [extract_beer_infos(beer_url) for beer_url in beer_urls]
